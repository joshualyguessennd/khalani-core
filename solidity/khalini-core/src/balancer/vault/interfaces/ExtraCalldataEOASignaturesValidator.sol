 ...
 */
abstract contract ExtraCalldataEOASignaturesValidator is
    EOASignaturesValidator
{
     ...
     */
    function _validateExtraCalldataSignature(address user, uint256 errorCode)
        internal
    {
     ...
     * This function returns bogus data if no signature is included.
     */
    function _decodeExtraCalldataWord(uint256 offset)
        private
        pure
        returns (bytes32 result)
    {
        ...
        assembly {
            result := calldataload(
                add(sub(calldatasize(), _EXTRA_CALLDATA_LENGTH), offset)
            )
        }
    }
}
