...
library BalanceAllocation {
     ...
     */
    function managedDelta(bytes32 newBalance, bytes32 oldBalance)
        internal
        pure
        returns (int256)
    {
        ...
        )
    {
        ...
        for (uint256 i = 0; i < results.length; i++) {
            ...
            lastChangeBlock_ = Math.max(
                lastChangeBlock_,
                lastChangeBlock(balance)
            );
        }
    }
    ...
    function toBalance(
        ...
        _require(
            _total >= _cash && _total < 2**112,
            Errors.BALANCE_TOTAL_OVERFLOW
        );
        ...
        returns (bytes32)
    {
        ...
        uint32 newLastChangeBlock = uint32(
            Math.max(
                lastChangeBlock(tokenABalance),
                lastChangeBlock(tokenBBalance)
            )
        );

        return
            _pack(cash(tokenABalance), cash(tokenBBalance), newLastChangeBlock);
    }
