    ...
    TwoTokenPoolsBalance
{
    ...
    function managePoolBalance(PoolBalanceOp[] memory ops)
        external
        override
        nonReentrant
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < ops.length; ++i) {
            ...
            (
                int256 cashDelta,
                int256 managedDelta
            ) = _performPoolManagementOperation(kind, poolId, token, amount);
    ...
    function _updateManagedBalance(
        ...
        if (specialization == PoolSpecialization.TWO_TOKEN) {
            managedDelta = _setTwoTokenPoolManagedBalance(
                poolId,
                token,
                amount
            );
        } else if (specialization == PoolSpecialization.MINIMAL_SWAP_INFO) {
            managedDelta = _setMinimalSwapInfoPoolManagedBalance(
                poolId,
                token,
                amount
            );
