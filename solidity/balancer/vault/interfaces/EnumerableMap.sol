...
library EnumerableMap {
    ...
    function set(
        ...
        if (keyIndex == 0) {
            ...
            map._entries[previousLength] = IERC20ToBytes32MapEntry({
                _key: key,
                _value: value
            });
     ...
     */
    function remove(IERC20ToBytes32Map storage map, IERC20 key)
        internal
        returns (bool)
    {
        // We read and store the key's index to prevent multiple reads from the same storage slot
        ...
        if (keyIndex != 0) {
            ...
            if (toDeleteIndex != lastIndex) {
                IERC20ToBytes32MapEntry storage lastEntry = map._entries[
                    lastIndex
                ];
     ...
     */
    function contains(IERC20ToBytes32Map storage map, IERC20 key)
        internal
        view
        returns (bool)
    {
     ...
     */
    function length(IERC20ToBytes32Map storage map)
        internal
        view
        returns (uint256)
    {
